...
df = pd.read_excel('IMD_2025.xlsx', sheet_name = 'IMD25')
...
df_clean = df[['Local Authority District name (2024)', 'Index of Multiple Deprivation (IMD) Rank', 'Index of Multiple Deprivation (IMD) Decile']]
...
results = df.groupby('Local Authority District name (2024)').size().reset_index(name = 'Total_LSOAs')
#print(results)
#Counting number of LSOAs with an IMD Decile 1 (top 10% most deprived areas in England), grouped by Local Authority District
decile1 = df[df['Index of Multiple Deprivation (IMD) Decile'] == 1].groupby('Local Authority District name (2024)').size()
#print(decile1)
#Adding this information to the results table
results['Total_In_Decile_1'] = results['Local Authority District name (2024)'].map(decile1).fillna(0)
#Checking output
#print(results)
...
decile123 = df[df['Index of Multiple Deprivation (IMD) Decile'] <= 3].groupby('Local Authority District name (2024)').size()

#Calculating percentage
results['Total_In_Deciles_123'] = results['Local Authority District name (2024)'].map(decile123).fillna(0)

results['Percentage_Deciles_123'] = (results['Total_In_Deciles_123']/results['Total_LSOAs']) * 100

#Output to Excel file
results.to_excel('IMD_2025_Decile123.xlsx', index = False)

#Generating summary statistics
df2 = pd.read_excel('IMD_2025_Decile123.xlsx')
